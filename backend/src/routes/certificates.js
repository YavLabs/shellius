...
const listQuerySchema = Joi.object({
  ...
  certType: Joi.string().valid('USER', 'HOST'),
  search: Joi.string().trim().max(200).allow(''),
  // Whitelisted — anything else is rejected here, before the service ever
  // sees it (certificateService.list's buildOrderBy mirrors this list).
  sortBy: Joi.string().valid('createdAt', 'serial', 'issuedTo', 'server', 'status', 'validBefore').default('createdAt'),
