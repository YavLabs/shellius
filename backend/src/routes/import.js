...
const TEMPLATES = {
  ...
  servers:
    ...
    // storeAsIdentity (true/false, default false): keep the password/key as a
    // reusable Keystore identity bound to this server, instead of wiping it
    // once onboarding finishes. identityName names it — rows sharing a name
    // share ONE identity, so a fleet behind one bastion key gets one entry.
    'hostname,displayName,ipAddress,dynamicIp,customer,environment,onboard,protocol,osType,osVersion,sshUser,password,keyFile,passphrase,sudoPassword,storeAsIdentity,identityName,rdpUsername,rdpPassword,labels,cloudProvider,cloudInstanceId,cloudRegion\n' +
    // root login with a key (no sudo needed)
    'web-01,Acme Prod Web,10.0.0.10,false,acme,prod,true,ssh,linux,Ubuntu 22.04,root,,keys/web-01.pem,,,false,,,,team:web;tier:frontend,aws,i-0abc123def,ap-south-1\n' +
    // sudo user with a key + sudo password
    'app-01,Acme App,10.0.0.11,false,acme,staging,true,ssh,linux,Ubuntu 22.04,ubuntu,,keys/app-01.pem,,SudoP@ss,true,Acme bastion key,,,tier:app,aws,i-0app111,ap-south-1\n' +
    // cloud VM with NO static IP (public IP changes on restart)
    'ephemeral-01,Ephemeral,10.0.0.40,true,acme,dev,true,ssh,linux,Ubuntu 22.04,ubuntu,LoginP@ss,,,SudoP@ss,false,,,,tier:scratch,aws,i-0eph222,ap-south-1\n' +
    // host that requires BOTH a key and a password to log in
    'bastion-01,Bastion,10.0.0.13,false,acme,prod,true,ssh,linux,Ubuntu 22.04,ubuntu,LoginP@ss,keys/bastion-01.pem,,SudoP@ss,true,Acme bastion key,,,tier:bastion,,,\n' +
    // encrypted private key (needs a passphrase) + sudo password
    'cache-01,Cache,10.0.0.14,false,acme,dev,true,ssh,linux,Ubuntu 22.04,ubuntu,,keys/cache-01.pem,KeyPassphrase,SudoP@ss,false,,,,tier:cache,,,\n' +
    // Windows host for RDP (no SSH fields)
    'win-01,Win Host,10.0.0.20,false,acme,prod,true,rdp,windows,Windows Server 2022,,,,,,false,,Administrator,Rdp!Pass,role:rdp,azure,vm-win-01,eastus\n' +
    // inventory only — added but NOT onboarded (no credentials needed)
    'inventory-01,Unmanaged Box,10.0.0.30,false,acme,dev,false,ssh,linux,Ubuntu 22.04,ubuntu,,,,,false,,,,note:not-onboarded,,,\n',
