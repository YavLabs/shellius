...
export async function listListeners(orgId, query = {}, scope = UNSCOPED) {
  ...
  const all = await prisma.hostListener.findMany({
    where,
  ...
  if (query.hasFindings === true || query.hasFindings === 'true') {
    ...
    return {
      ...
      meta: { total, page, limit, truncated: all.length >= MAX_ROWS },
    };
  }
...
export async function listServices(orgId, query = {}, scope = UNSCOPED) {
  ...
  const rows = await attachFindings(orgId, all);
  ...
  return {
    ...
    meta: {
      ...
      servers: new Set(rows.map((r) => r.serverId)).size,
